...
export function InvoiceManagement() {
  ...
  const { register, handleSubmit, reset, setValue, watch } = useForm({
    defaultValues: {
      ...
      amount: '',
      supplier: '',
      description: '' // Remove this field completely
    }
  });
          ...
          {isLoading ? (
            ...
            <div className="space-y-4">
              {Array.isArray(invoices) ? invoices.slice(0, 3).map((invoice: any) => (
                <div key={invoice.id} className="bg-white/10 dark:bg-black/10 rounded-lg p-3 sm:p-4 border-l-4 border-primary">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start space-y-2 sm:space-y-0">
                    <div className="flex-1">
                      <p className="font-semibold text-foreground text-sm sm:text-base">{invoice.description}</p>
                      <p className="text-xs sm:text-sm text-muted-foreground">
                        {new Date(invoice.date).toLocaleDateString('pt-PT')}
                      </p>
                      <Badge className={`mt-2 text-xs ${getStatusBadge(invoice.status)}`}>
                        {invoice.status === 'approved' ? 'Aprovada' : 
                         invoice.status === 'rejected' ? 'Rejeitada' : 'Pendente'}
                      </Badge>
                    </div>
                    <p className="font-bold text-foreground text-base sm:text-lg self-start sm:self-auto">€{parseFloat(invoice.amount).toFixed(2)}</p>
                  </div>
                </div>
              )) : null}
            </div>
          ...
          <div className="mt-6 pt-6 border-t border-white/20 dark:border-gray-600">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="text-center">
                <p className="text-lg sm:text-2xl font-bold text-foreground">€{totalAmount.toFixed(2)}</p>
                <p className="text-xs sm:text-sm text-muted-foreground">Total este mês</p>
              </div>
              <div className="text-center">
                <p className="text-lg sm:text-2xl font-bold text-green-600">€{approvedAmount.toFixed(2)}</p>
                <p className="text-xs sm:text-sm text-muted-foreground">Aprovado</p>
              </div>
            </div>
          </div>
        </CardContent>
